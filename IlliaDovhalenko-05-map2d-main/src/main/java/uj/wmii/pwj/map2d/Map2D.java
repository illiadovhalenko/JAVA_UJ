...
public interface Map2D<R, C, V> {
    ...
    static <R, C, V> Map2D<R, C, V> createInstance() {
        return new Map2D<>() {
            record Key<R, C>(R rowKey, C columnKey) {}

            final Map<Key<R, C>, V> mainMap = new HashMap<>();
            ...
            public V put(R rowKey, C columnKey, V value) {
                ...
                Key<R, C> key = new Key<>(rowKey, columnKey);
                if (mainMap.containsKey(key)) {
                    V previousValue = mainMap.get(key);
                    mainMap.replace(key, value);
            ...
            public Map<C, V> rowView(R rowKey) {
                ...
                for (var entry : mainMap.entrySet()) {
                    if (entry.getKey().rowKey.equals(rowKey))
                        resultMap.put(entry.getKey().columnKey, entry.getValue());
                }
            ...
            public Map<R, V> columnView(C columnKey) {
                ...
                for (var entry : mainMap.entrySet()) {
                    if (entry.getKey().columnKey.equals(columnKey))
                        resultMap.put(entry.getKey().rowKey, entry.getValue());
                }
                return Collections.unmodifiableMap(resultMap);
            }
            ...
            public boolean hasKey(R rowKey, C columnKey) {
                return mainMap.containsKey(new Key<>(rowKey, columnKey));
            }

            @Override
            public boolean hasRow(R rowKey) {
                for (Key<R, C> key : mainMap.keySet()) {
                    if (key.rowKey.equals(rowKey))
                        return true;
                }
                return false;
            }
            ...
            public boolean hasColumn(C columnKey) {
                for (Key<R, C> key : mainMap.keySet()) {
                    if (key.columnKey.equals(columnKey))
                        return true;
                }
                return false;
            }
            ...
            public Map<R, Map<C, V>> rowMapView() {
                Map<R, Map<C, V>> resultMap = new HashMap<R, Map<C, V>>();
                Set<R> setRow=new HashSet<>();
                for (Key<R, C> key : mainMap.keySet()) {
                    setRow.add(key.rowKey);
                }
                for(R row: setRow){
                    resultMap.put(row, rowView(row));
                }
                return Collections.unmodifiableMap(resultMap);
            }
            ...
            public Map<C, Map<R, V>> columnMapView() {
                Map<C, Map<R, V>> resultMap = new HashMap<C, Map<R, V>>();
                Set<C> setColumn=new HashSet<>();
                for (Key<R, C> key : mainMap.keySet()) {
                    setColumn.add(key.columnKey);
                }
                for(C column: setColumn){
                    resultMap.put(column, columnView(column));
                }
                return Collections.unmodifiableMap(resultMap);
            }
            ...
            public Map2D<R, C, V> fillMapFromRow(Map<? super C, ? super V> target, R rowKey) {
                Map<C, V> mapCV = rowView(rowKey);
                for (C key : mapCV.keySet()) {
                    target.put(key, mapCV.get(key));
                }
//                target.putAll(rowView(rowKey));
            ...
            public Map2D<R, C, V> fillMapFromColumn(Map<? super R, ? super V> target, C columnKey) {
                Map<R, V> mapRV = columnView(columnKey);
                for (R key : mapRV.keySet()) {
                    target.put(key, mapRV.get(key));
                }
 //               target.putAll(columnView(columnKey));
            ...
            public Map2D<R, C, V> putAll(Map2D<? extends R, ? extends C, ? extends V> source) {
                for (var rcv : source.rowMapView().entrySet()) {
                    for (var cv : rcv.getValue().entrySet()) {
                        mainMap.put(new Key<>(rcv.getKey(), cv.getKey()), cv.getValue());
                    }
                }
                return this;
            }
            ...
            public Map2D<R, C, V> putAllToRow(Map<? extends C, ? extends V> source, R rowKey) {
                for (C columnKey : source.keySet()) {
                    mainMap.put(new Key<>(rowKey, columnKey), source.get(columnKey));
                }
                return this;
            }
            ...
            public Map2D<R, C, V> putAllToColumn(Map<? extends R, ? extends V> source, C columnKey) {
                for (R rowKey : source.keySet()) {
                    mainMap.put(new Key<>(rowKey, columnKey), source.get(rowKey));
                }
                return this;
            }
            ...
            public <R2, C2, V2> Map2D<R2, C2, V2> copyWithConversion(Function<? super R, ? extends R2> rowFunction, Function<? super C, ? extends C2> columnFunction, Function<? super V, ? extends V2> valueFunction) {
                Map2D<R2, C2, V2> v2Map2D = Map2D.createInstance();
                for (var entry : mainMap.entrySet()) {
                    v2Map2D.put(rowFunction.apply(entry.getKey().rowKey), columnFunction.apply(entry.getKey().columnKey), valueFunction.apply(entry.getValue()));
                }
                return v2Map2D;
            }
        };
